...
上記がAWSを使用する上でのメリットと考え学習しています。
オンプレとクラウドを比較した際のオンプレインフラ構築は複雑であり、デプロイするまでに非常に時間がかかる。
クラウドインフラの場合はワンクリックで新しいインフラを準備できる上、最悪壊して再構築することは比較的簡単にできる、というのが最大のメリットではないかと考えています。
